    ...
    const { selectedAgents } = await inquirer.prompt([
      {
        type: 'checkbox',
        name: 'selectedAgents',
        message: '🤖 Select one or more agent profiles to copy:',
        choices: agents,
        default: ['claude.md'],
        validate: (selected) => selected.length > 0 || 'Please select at least one agent.',
      },
    ]);
    await fs.ensureDir(targetPath);
    for (const agentFile of selectedAgents) {
      const sourceFile = path.join(baseTemplatePath, 'dev', agentFile);
      const destFile = path.join(targetPath, agentFile);
      await fs.copy(sourceFile, destFile);
    }
    console.log(`✅ Copied agent profiles to .dokugent/conventions/${type}: ${selectedAgents.join(', ')}`);
  } else {
    const templatePath = resolveVersionedPath(baseTemplatePath, type.split('@')[0]);
