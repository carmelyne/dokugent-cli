...
export async function generatePreview(agentKey, variantKey) {
  ...
  for (const file of files) {
    ...
    // Use dynamic import for crypto in ESM context
    const cryptoModule = await import('crypto');
    const sha = cryptoModule.createHash('sha256').update(buffer).digest('hex');
    shaLines.push(`${sha}  ${file}`);
  }

  await fs.writeFile(path.join(previewPath, 'preview.sha256'), shaLines.join('\n'), 'utf8');
  console.log('🔒 Preview files are now read-only and a SHA256 integrity file has been generated for tamper detection.');
}
