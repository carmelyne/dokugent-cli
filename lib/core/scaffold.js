...
function scaffoldTemplates(scope, options) {
  ...
  const templateBase = path.resolve(__dirname, '../../presets/templates/protocols');
  ...
  expectedDefaultFiles.forEach(templateRelPath => {
    const src = path.resolve(__dirname, '../../presets/templates', templateRelPath);
  ...
  if (!fs.existsSync(llmLoadPath)) {
    const defaultYaml = expectedDefaultFiles.map(templateRelPath =>
      templateRelPath.replace(/^presets\/templates\/protocols\//, '')
    );
  ...
  const planTemplatePath = path.resolve(__dirname, '../../presets/templates/plan.yaml');
  const planDestPath = path.join(rootPath, 'plan.yaml');
  if (!fs.existsSync(planDestPath)) {
    if (fs.existsSync(planTemplatePath)) {
      fs.copyFileSync(planTemplatePath, planDestPath);
      console.log('📋 Copied scaffolded plan.yaml');
    } else {
      fs.writeFileSync(planDestPath, '');
      console.warn('⚠️ No plan.yaml template found. Created empty plan.yaml');
    }
  }
