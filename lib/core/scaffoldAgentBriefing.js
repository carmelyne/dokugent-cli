...
function scaffoldAgentBriefing(llm = 'claude') {
  ...
  const basePath = path.resolve('.docugent/agent-briefings');
  ...
  Object.entries(folders).forEach(([folder, files]) => {
    files.forEach(file => {
      const filePath = path.join('.docugent', folder, file);
  ...
  const llmConfigPath = path.join('.docugent', 'llm-load.yml');
  ...
  console.log(`🧠 Agent briefing created: agent-briefings/${llm}.md`);
