...
function scaffoldApp(scope = 'core', options = {}) {
  // Handle --custom= flag before scaffolding templates
  if (options.custom) {
    if (options.withChecklists || options.force || options.backup) {
      console.error('❌ Error: The --custom flag must be used on its own. Please remove other flags.');
      return;
    }
    ...
    console.log(`📁 Custom Folder created: .dokugent/${options.custom}/`);
    return;
  }
