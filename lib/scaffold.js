...
function scaffoldApp(scope = 'core', options = {}) {
  ...
  Object.entries(foldersToCreate).forEach(([folder, files]) => {
    ...
    files.forEach(file => {
      ...
      if (options.withChecklists && templateExists) {
        ...
        console.warn(`⚠️  Fallback to blank: ${relativePath}`);
      }
    });
  ...
  // Create empty files to match structure in presets/templates/
  const emptyFiles = [
    'db-schema/models.md',
  ...
  emptyFiles.forEach(file => {
    const filePath = path.join(__dirname, '../presets/templates', file);
